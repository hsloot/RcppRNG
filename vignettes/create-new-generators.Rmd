...
First, we have to tell `Rcpp` that we are using `c++11` and that our code depends links to the 
...
ErlangGenerator<T>::ErlangGenerator() : 
    param_(), exp_generator_(ExpGenerator<T>(ExpDistribution(1. / param_.rate()))) {
}

template<typename T>
ErlangGenerator<T>::ErlangGenerator(const ErlangDistribution& param) : 
    param_(param), exp_generator_(ExpGenerator<T>(ExpDistribution(1. / param_.rate()))) {}
